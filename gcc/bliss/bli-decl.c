...
pop_scope (void)
{
  ...
  for (b = scope->bindings; b; b = free_binding_and_advance (b))
	...
	case ERROR_MARK:
	  ...
	  if (b->id)
	    {
	      // check	      gcc_assert (I_SYMBOL_BINDING (b->id) == b);
...
c_write_global_declarations (void)
{
  ...
  /* Don't waste time on further processing if -fsyntax-only or we've
     encountered errors.  */
  if (flag_syntax_only || errorcount || sorrycount /*|| cpp_errors (parse_in)*/) // check
    return;
  return;
...
push_parm_decl_init (tree parm, tree init)
{
  ...
  DECL_LANG_SPECIFIC(decl)=init; // borrow this field?
...
{
  return lookup_tag(LINKAGE, t, 1);
}

void
reset_symbol(tree t)
{
  I_SYMBOL_BINDING(t) = 0;
}

tree
get_label_binding(tree t)
{
  return I_LABEL_BINDING(t);
}
