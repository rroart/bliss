...
convert_arguments (tree typelist, tree values, tree function, tree fundecl)
{
  ...
  for (valtail = values, typetail = typelist, parmnum = 0;
      ...
      if (type != 0)
	{
	  ...
	  if (!COMPLETE_TYPE_P (type))
	    type = long_integer_type_node; // check. 64-bit
		  ...
		  tree else_block, bool nested_if)
{
  ...
  stmt = build3 (COND_EXPR, long_integer_type_node /*void_type_node*/, cond, then_block, else_block); // check 64-bit
...
c_finish_loop (location_t start_locus, tree cond, tree incr, tree body,
	       tree blab, tree clab, bool cond_is_first, tree loop_value, tree block_value)
{
  tree entry = NULL, exit = NULL, t;
  tree ret = NULL;
  tree decl = loop_value;
  ...
  if (body)
    add_stmt (body);
  if (block_value) {
    //decl = create_temp_var ();
    //decl = create_tmp_var (long_integer_type_node);
    //decl2 = create_tmp_var_raw (long_integer_type_node, 0);  
    tree value = block_value; // BIND_EXPR_BODY (body);
    tree t = value;
    
#if 0
    tree t2 = build (TARGET_EXPR, TREE_TYPE (decl), decl, 0, 0, NULL_TREE);
    TREE_SIDE_EFFECTS (t2) = 1;
    add_stmt (t2);
#endif

    if (t && !(TREE_CODE(t)==VAR_DECL && DECL_ARTIFICIAL (decl))) 
#if 0
		 ...
		 int convert_p)
{
  ...
  if (code==PLUS_EXPR || code==MINUS_EXPR) {
    ...
    // not yet?
#ifdef __x86_64__
    if (POINTER_TYPE_CHECK(orig_op0))
      orig_op0 = convert (long_integer_type_node, orig_op0);
    if (POINTER_TYPE_CHECK(orig_op1))
      orig_op1 = convert (long_integer_type_node, orig_op1);
#else
#if 1
    if (POINTER_TYPE_CHECK(orig_op0))
      orig_op0 = convert (long_integer_type_node, orig_op0);
...
#else
  if (POINTER_TYPE_CHECK(orig_op0))
    orig_op0 = convert (integer_type_node, orig_op0);
 if (POINTER_TYPE_CHECK(orig_op1))
   orig_op1 = convert (integer_type_node, orig_op1);
#endif
#endif
#endif
  }
...
{
  struct c_switch *cs = c_switch_stack;
  tree mystart=fold(/*parser_*/build_binary_op(MINUS_EXPR,cs->start,build_int_cst (long_integer_type_node, 1),1)); // check -parser
  tree myend=fold(/*parser_*/build_binary_op(PLUS_EXPR,cs->end,build_int_cst (long_integer_type_node, 1),1)); // check -parser
  do_case (build_int_cst (long_integer_type_node, -0x7fffffff), mystart);
  return do_case (myend, build_int_cst (long_integer_type_node, 0x7fffffff));
}
